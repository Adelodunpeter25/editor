...
enum NestableToken: Equatable, Hashable, Sendable {
  ...
  case pair(
    Pair<String>, prefixes: [String] = [], isMultiline: Bool, isNestable: Bool,
...
extension [NestableToken: SyntaxType] {
  ...
  func parseHighlights(in string: String, range parseRange: NSRange) throws -> [SyntaxType:
    [NSRange]]
  {

    let positions: [NestableItem] =
      try self
      ...
      .filter { item in
        switch item.token {
        case .pair(let pair, _, _, _, let escapeCharacter?)
        where String(escapeCharacter) != pair.end:
    ...
    while index < positions.endIndex {
      ...
      guard
        let endIndex = beginPosition.token.matchingEndIndex(
          in: positions, from: index, upperBound: searchUpperBound)
      else {
        if !beginPosition.token.allowsMultiline {
...
extension NestableToken {

  /// Whether this token can span multiple lines.
  fileprivate var allowsMultiline: Bool {
  ...
  fileprivate var allowsNesting: Bool {
  ...
  fileprivate func positions(in string: String, type: SyntaxType, range parseRange: NSRange)
    -> [NestableItem]
  {
    ...
    case .inline(let delimiter, let leadingOnly):
      return
        ...
        ? try! NSRegularExpression(
          pattern: "^ *(\(NSRegularExpression.escapedPattern(for: delimiter)))",
        ...
        .filter { range in
          ...
          range.length > 1 || range.lowerBound == 0
            || Unicode.Scalar((string as NSString).character(at: range.lowerBound - 1))?.properties
              .isWhitespace == true
        }
      ...
      if pair.begin == pair.end {
        ...
        if prefixes.isEmpty {
          ...
          return ranges.map { range in
            if let prefixLength = Self.matchingPrefixLength(
              at: range.location, in: nsString, prefixes: prefixes, parseRange: parseRange)
            {
              NestableItem(
                type: type, token: self, role: .begin,
                range: NSRange(
                  location: range.location - prefixLength, length: range.length + prefixLength))
            } else {
        ...
        let beginItems: [NestableItem] =
          if prefixes.isEmpty {
            ...
            string.ranges(of: pair.begin, range: parseRange)
              .compactMap { range in
                guard
                  let prefixLength = Self.matchingPrefixLength(
                    at: range.location, in: nsString, prefixes: prefixes, parseRange: parseRange)
                else { return nil }
                return NestableItem(
                  type: type, token: self, role: .begin,
  ...
  fileprivate func matchingEndIndex(
    in positions: [NestableItem], from startIndex: Int, upperBound searchUpperBound: Int
  ) -> Int? {

    let isDoubleEscape: Bool =
      switch self {
    ...
    for (offset, position) in positions[startIndex...].enumerated() where position.token == self {
      ...
      if position.role.contains(.end) {
        ...
        if isDoubleEscape, startIndex < positions.count {
          skipCount =
            positions[(startIndex + offset + 1)...].prefix { next in
              next.token == position.token && next.role.contains(.end)
  ...
  private static func matchingPrefixLength(
    at location: Int, in nsString: NSString, prefixes: [String], parseRange: NSRange
  ) -> Int? {
