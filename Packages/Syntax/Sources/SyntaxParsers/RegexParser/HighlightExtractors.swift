  ...
  var extractor: any HighlightExtractable {
    ...
    get throws {
      ...
      case (true, .some(let end)):
        try BeginEndRegularExpressionExtractor(
          beginPattern: self.begin, endPattern: end, ignoresCase: self.ignoreCase,
          isMultiline: self.isMultiline)

      case (true, .none):
        try RegularExpressionExtractor(
          pattern: self.begin, ignoresCase: self.ignoreCase, isMultiline: self.isMultiline)

      case (false, .some(let end)):
        BeginEndStringExtractor(
          begin: self.begin, end: end, ignoresCase: self.ignoreCase, isMultiline: self.isMultiline)
...
struct BeginEndStringExtractor: HighlightExtractable {
  ...
  func ranges(in string: String, range: NSRange) throws -> [NSRange] {
    ...
    while location < range.upperBound {
      // find start string
      let beginRange = (string as NSString).range(
        of: self.begin, options: self.options, range: NSRange(location..<range.upperBound))
      ...
      let upperBound =
        ...
        ? range.upperBound
        : min(
          range.upperBound, (string as NSString).lineContentsEndIndex(at: beginRange.upperBound))
      let endRange = (string as NSString).range(
        of: self.end, options: self.options, range: NSRange(location..<upperBound))
...
struct RegularExpressionExtractor: HighlightExtractable {
  ...
  func ranges(in string: String, range: NSRange) throws -> [NSRange] {
    ...
    try self.regex.cancellableMatchRanges(
      in: string, options: [.withTransparentBounds, .withoutAnchoringBounds], range: range)
  }
}
...
struct BeginEndRegularExpressionExtractor: HighlightExtractable {
  ...
  func ranges(in string: String, range: NSRange) throws -> [NSRange] {
    ...
    while location < range.upperBound {
      ...
      // find start pattern
      let beginRange = self.beginRegex.rangeOfFirstMatch(
        in: string, options: options, range: NSRange(location..<range.upperBound))
      ...
      // find end pattern
      let endRange = self.endRegex.rangeOfFirstMatch(
        in: string, options: options, range: NSRange(searchStartIndex..<upperBound))
