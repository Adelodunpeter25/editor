...
extension OutlineItem.IdentityResolver {
  ...
  fileprivate func reusingStableIDs(in items: [OutlineItem]) -> [OutlineItem] {
    ...
    let difference =
      (max(currentKeys.count, previousKeys.count) <= Self.moveInferenceItemLimit)
    ...
    self.assignIDsForStableSequence(
      changedOffsets: changedOffsets, assignmentState: &assignmentState)
  ...
  private func assignID(
    from oldOffset: Int, to newOffset: Int, assignmentState: inout AssignmentState
  ) {
  ...
  private func assignIDsForMoves(
    in difference: CollectionDifference<IdentityKey>, assignmentState: inout AssignmentState
    ...
    for change in difference {
      switch change {
      case .remove(let oldOffset, _, _):
        removedOldOffsets.insert(oldOffset)
      case .insert(let newOffset, _, let oldOffset?):
      ...
      case .insert(let newOffset, _, nil):
        insertedNewOffsets.insert(newOffset)
      }
    }
  ...
  ///   - assignmentState: The state that tracks assignments while resolving IDs.
  private func assignIDsForStableSequence(
    changedOffsets: (removed: IndexSet, inserted: IndexSet), assignmentState: inout AssignmentState
  ) {
  ...
  private func assignRemainingIDsByKey(assignmentState: inout AssignmentState) {
    ...
    for (oldOffset, item) in self.previousItems.enumerated()
    where !assignmentState.usedOldOffsets.contains(oldOffset) {
      remainingOffsetsByKey[IdentityKey(item), default: IndexSet()].insert(oldOffset)
    }

    var lastUsedOldOffset = -1
    for newOffset in assignmentState.resolvedItems.indices
    where assignmentState.assignedOldOffsets[newOffset] == nil {
      ...
      guard
        ...
        let oldOffset = candidateOffsets.integerGreaterThan(lastUsedOldOffset)
          ?? candidateOffsets.first
      else { continue }
