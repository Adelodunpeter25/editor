...
actor TreeSitterClient: IncrementalParsing, HighlightParsing, OutlineParsing {
  ...
  init(
    ...
    languageProvider: @escaping LanguageLayer.LanguageProvider, syntax: TreeSitterSyntax,
    maximumParseLength: Int = TreeSitterClient.maximumParseLength
  ) throws {
  ...
  func noteEdit(editedRange: NSRange, delta: Int, insertedText: String) throws {
    ...
    let edit = try self.content.applyEdit(
      editedRange: editedRange, delta: delta, insertedText: insertedText)
  ...
  func parseHighlights(in string: String, range: NSRange) async throws -> (
    ...
    let invalidations =
      !self.pendingAffectedRanges.isEmpty
      ? self.layer.parse(
        with: content, affecting: self.pendingAffectedRanges.indexSet, resolveSublayers: true)
      : []
  ...
  private func resetLayer() {
    ...
    self.layer = try! LanguageLayer(
      languageConfig: self.languageConfig, configuration: self.layerConfiguration)
    self.pendingAffectedRanges.clear()
  ...
  private func queryMatches(_ definition: Query.Definition, in range: NSRange, string: NSString)
    throws -> [QueryMatch]
  {
...
extension [QueryCapture] {
  ...
  fileprivate func resolvingCaptureConflicts() -> [QueryCapture] {
    ...
    for (index, capture) in self.enumerated() {
      let key = CaptureNodeKey(
        depth: capture.depth, location: capture.range.location, length: capture.range.length)
...
extension Highlight {
  ...
  fileprivate init?(capture: QueryCapture) {
...
extension IndexSet {
  ...
  fileprivate func unionRange() -> NSRange? {
