...
extension TreeSitterClient.Content {
  ...
  mutating func applyEdit(editedRange: NSRange, delta: Int, insertedText: String) throws
    -> InputEdit
  {
    ...
    let editsCRLFBoundary =
      preEditString.hasCRLFBoundary(at: preEditRange.lowerBound)
    ...
    self.updateLineStartIndexes(
      preEditRange: preEditRange, editedRange: editedRange, delta: delta,
      insertedText: insertedText, editsCRLFBoundary: editsCRLFBoundary)

    return InputEdit(
      startByte: preEditRange.lowerBound * 2,
  ...
  private mutating func updateLineStartIndexes(
    preEditRange: NSRange, editedRange: NSRange, delta: Int, insertedText: String,
    editsCRLFBoundary: Bool
  ) {

    let string = self.string as NSString
    if editsCRLFBoundary || string.hasCRLFBoundary(at: editedRange.lowerBound)
    ...
    lineStarts.reserveCapacity(
      self.lineStarts.count - (shiftStartIndex - removalStartIndex) + insertedLineStarts.count)
...
extension NSString {
  ...
  fileprivate func lineStartIndexes() -> [Int] {
  ...
  fileprivate func hasCRLFBoundary(at location: Int) -> Bool {
