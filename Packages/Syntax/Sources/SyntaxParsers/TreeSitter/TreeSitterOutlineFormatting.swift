...
extension TreeSitterOutlineFormatting {
  ...
  static func item(for match: QueryMatch, source: NSString, policy: OutlinePolicy) -> OutlineItem? {
    ...
    return OutlineItem(
      title: displayTitle, range: range, kind: capture.kind, indent: .level(capture.depth))
  }
  ...
  ///   - source: The source text as `NSString`.
  /// - Returns: The raw capture text and range.
  static func defaultTitle(capture: OutlineCapture, source: NSString) -> (
    title: String, range: NSRange
  ) {
  ...
  func depth(captureNameComponents components: [String], captureNode node: Node?) -> Int {
    ...
    return sequence(first: node, next: \.parent).filter { node in
      self.ignoredDepthNodeTypes.isEmpty
        || !self.ignoredDepthNodeTypes.contains(node.nodeType ?? "")
    }.count
  }
