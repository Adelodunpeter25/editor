...
public struct SyntaxMappingTable: Equatable, Sendable {
  ...
  public init(
    extensions: [String: [SyntaxName]] = [:], filenames: [String: [SyntaxName]] = [:],
    interpreters: [String: [SyntaxName]] = [:]
  ) {
  ...
  public init(syntaxNames: [SyntaxName], maps: [SyntaxName: Syntax.FileMap]) {
    ...
    self.extensions = Self.buildMapping(
      for: \.extensions, syntaxNames: syntaxNames, maps: maps, normalizesKeys: true)
  ...
  private static func buildMapping(
    for keyPath: KeyPath<Syntax.FileMap, [String]?>, syntaxNames: [SyntaxName],
    maps: [SyntaxName: Syntax.FileMap], normalizesKeys: Bool = false
  ) -> [String: [String]] {
