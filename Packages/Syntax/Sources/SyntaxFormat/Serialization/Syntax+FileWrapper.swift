...
extension Syntax {
  ...
  public init(fileWrapper: FileWrapper) throws {
    ...
    self.characterDelimiters = edit?.characterDelimiters ?? []
    self.indentation = edit?.indentation ?? .init()

    self.completions =
      try fileWrapper.fileWrappers?[Filename.completion]?.regularFileContents
    ...
    if let wrapper = fileWrapper.fileWrappers?[Filename.regex] {
      self.highlights =
        try wrapper.fileWrappers?[Filename.highlights]?.regularFileContents
        .map { try decoder.decode([SyntaxType: [Highlight]].self, from: $0) } ?? [:]
      self.outlines =
        try wrapper.fileWrappers?[Filename.outlines]?.regularFileContents
  ...
  public var fileWrapper: FileWrapper {
    ...
    get throws {
      ...
      if !self.commentDelimiters.isEmpty || !self.stringDelimiters.isEmpty
        || !self.characterDelimiters.isEmpty || !self.indentation.isEmpty
      {
        let edit = Edit(
          ...
          characterDelimiters: self.characterDelimiters.isEmpty ? nil : self.characterDelimiters,
...
extension Syntax.FileMap {
  ...
  public static func load(at urls: [URL], ignoresInvalidData: Bool = false) throws -> [String:
    Syntax.FileMap]
  {
