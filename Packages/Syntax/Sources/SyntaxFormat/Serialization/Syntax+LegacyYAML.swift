...
extension Syntax {
  ...
  @discardableResult public static func migrateFormat(
    in directoryURL: URL, to destinationURL: URL? = nil, deletingOriginal: Bool = true,
    shouldMigrate: (_ name: String) -> Bool = { _ in true }
  ) throws -> Int {
    ...
    for url in urls {
      ...
      do {
        ...
        Logger.syntaxFormat.error(
          "Failed migrating syntax definition \"\(url.lastPathComponent)\": \(error)")
        continue
      }
      count += 1
    }
  ...
  public static func migrate(
    fileURL: URL, to directoryURL: URL? = nil, deletingOriginal: Bool = true
  ) throws {
  ...
  /// - Parameters:
  ///   - yamlData: YAML data.
  public init(yamlData: Data) throws {
...
extension Logger {
  ...
  fileprivate static let syntaxFormat = Logger(
    subsystem: "com.coteditor.CotEditor", category: "syntax-format")
}
...
extension Syntax: Decodable {
  ...
  private struct LegacyHighlight: Decodable {
    ...
    init(from decoder: any Decoder) throws {
      ...
      self.highlight = Highlight(
        ...
        isRegularExpression: try container.decodeIfPresent(Bool.self, forKey: .isRegularExpression)
          ?? false,
  ...
  public init(from decoder: any Decoder) throws {
    ...
    var fileMap = FileMap()
    fileMap.extensions =
      try values.decodeIfPresent([KeyString].self, forKey: .extensions)?.compactMap(\.keyString)
      ?? []
    fileMap.filenames =
      try values.decodeIfPresent([KeyString].self, forKey: .filenames)?.compactMap(\.keyString)
      ?? []
    fileMap.interpreters =
      try values.decodeIfPresent([KeyString].self, forKey: .interpreters)?.compactMap(\.keyString)
      ?? []
    self.fileMap = fileMap

    var highlights: [SyntaxType: [Highlight]] = [:]
    highlights[.keywords] =
      try values.decodeIfPresent([LegacyHighlight].self, forKey: .keywords)?.map(\.highlight) ?? []
    highlights[.commands] =
      try values.decodeIfPresent([LegacyHighlight].self, forKey: .commands)?.map(\.highlight) ?? []
    highlights[.types] =
      try values.decodeIfPresent([LegacyHighlight].self, forKey: .types)?.map(\.highlight) ?? []
    highlights[.attributes] =
      try values.decodeIfPresent([LegacyHighlight].self, forKey: .attributes)?.map(\.highlight)
      ?? []
    highlights[.variables] =
      try values.decodeIfPresent([LegacyHighlight].self, forKey: .variables)?.map(\.highlight) ?? []
    highlights[.values] =
      try values.decodeIfPresent([LegacyHighlight].self, forKey: .values)?.map(\.highlight) ?? []
    highlights[.numbers] =
      try values.decodeIfPresent([LegacyHighlight].self, forKey: .numbers)?.map(\.highlight) ?? []
    highlights[.strings] =
      try values.decodeIfPresent([LegacyHighlight].self, forKey: .strings)?.map(\.highlight) ?? []
    highlights[.characters] =
      try values.decodeIfPresent([LegacyHighlight].self, forKey: .characters)?.map(\.highlight)
      ?? []
    highlights[.comments] =
      try values.decodeIfPresent([LegacyHighlight].self, forKey: .comments)?.map(\.highlight) ?? []

    // migrate delimiter pairs from highlights to dedicated delimiter properties
    let (stringHighlights, stringDelimiters) = Self.extractDelimiters(
      from: highlights[.strings] ?? [])
    let (characterHighlights, characterDelimiters) = Self.extractDelimiters(
      from: highlights[.characters] ?? [])
    ...
    self.outlines =
      (try values.decodeIfPresent([LegacyOutline].self, forKey: .outlines)?.map(\.outline) ?? [])
    ...
    self.commentDelimiters =
      (try values.decodeIfPresent([String: String].self, forKey: .commentDelimiters))
    ...
    self.completions =
      try values.decodeIfPresent([KeyString].self, forKey: .completions)?
  ...
  private static func extractDelimiters(from highlights: [Highlight]) -> (
    [Highlight], [PairDelimiter]
  ) {
    ...
    for highlight in highlights {
        ...
        highlight.begin.allSatisfy("\"'`".contains)
      {
        delimiters.append(
          PairDelimiter(
            begin: highlight.begin, end: end, escapeCharacter: "\\",
...
extension Syntax.Comment {
  ...
  fileprivate init(legacyDictionary dictionary: [String: String]) {
