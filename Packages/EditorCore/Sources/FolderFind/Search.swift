...
struct Search {
  ...
  init(
    rootURL: URL, query: FolderFind.Query, options: FolderFind.Options,
    progress: FolderFindProgress?, isIncluded: (@Sendable (FolderFind.Candidate) -> Bool)?
  ) throws {
  ...
  private mutating func searchDirectory(at directoryURL: URL) async throws {
    ...
    guard self.visitedDirectories.insert(directoryURL.resolvingSymlinksInPath()).inserted else {
      return
    }
    ...
    guard
      let urls = try? FileManager.default.contentsOfDirectory(
        at: directoryURL,
        includingPropertiesForKeys: Array(FolderFind.Candidate.metadataResourceKeys))
    else { return }
  ...
  private mutating func searchFile(_ candidate: FolderFind.Candidate) throws {
    ...
    guard
      !candidate.contentType.conforms(to: .propertyList)
        || !Self.isBinaryPropertyList(at: candidate.fileURL)
    else {
      self.recordSkippedFile()
      return
    }
    ...
    do {
      string = try String(
        contentsOf: candidate.fileURL, decodingOptions: self.options.decodingOptions)
    } catch {
      self.recordSkippedFile()
      return
    }
    ...
    let directoryPathComponents = candidate.fileURL.deletingLastPathComponent().standardizedFileURL
      .pathComponents

    guard directoryPathComponents.starts(with: rootPathComponents) else {
      return assertionFailure()
    }

    self.files.append(
      FolderFind.FileResult(
  ...
  private func includes(_ candidate: FolderFind.Candidate) -> Bool {
    ...
    let includesFileType =
      self.isIncluded?(candidate)
      ?? (self.options.includesOtherFileTypes || FolderFind.isSearchableText(candidate))
  ...
  /// - Throws: `CancellationError` if the task is cancelled.
  private func matches(in string: String, using textFind: TextFind, maximumLineLength: Int = 1024)
    throws -> [FolderFind.Match]
  {
