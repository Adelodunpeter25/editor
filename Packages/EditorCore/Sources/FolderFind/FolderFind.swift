...
public enum FolderFind {
  ...
  public struct Candidate: Equatable, Sendable {
    ...
    static let metadataResourceKeys: Set<URLResourceKey> = [
      .contentTypeKey, .isDirectoryKey, .isHiddenKey,
    ]
  ...
  public static func find(
    in rootURL: URL, query: Query, options: Options = .init(), progress: FolderFindProgress? = nil,
    isIncluded: (@Sendable (Candidate) -> Bool)? = nil
  ) async throws -> Summary {

    // validate search conditions before traversing the folder
    try query.validate()

    var search = try Search(
      rootURL: rootURL, query: query, options: options, progress: progress, isIncluded: isIncluded)
...
extension FolderFind.Summary {
  ...
  public func result(for id: FolderFind.ResultID) -> FolderFind.Result? {
  ...
  public mutating func removeResult(for id: FolderFind.ResultID) {
  ...
  @discardableResult public mutating func updateMatchRanges(
    in fileURL: URL, editedRange: NSRange, changeInLength: Int, length: Int
  ) -> Bool {

    guard let fileIndex = self.files.firstIndex(where: { $0.fileURL == fileURL }) else {
      return false
    }
    ...
    for matchIndex in self.files[fileIndex].matches.indices {
      didUpdate =
        ...
        .matches[matchIndex]
        .updateRange(editedRange: editedRange, changeInLength: changeInLength, length: length)
        || didUpdate
    }
...
extension FolderFind.Match {
  ...
  fileprivate mutating func updateRange(editedRange: NSRange, changeInLength: Int, length: Int)
    -> Bool
  {
    ...
    let newLowerBound = Self.updatedLowerBound(
      of: self.range, oldEditedRange: oldEditedRange,
      editedRange: editedRange, changeInLength: changeInLength)
    let newUpperBound = Self.updatedUpperBound(
      of: self.range, oldEditedRange: oldEditedRange,
      editedRange: editedRange, changeInLength: changeInLength)
  ...
  private static func updatedLowerBound(
    of range: NSRange, oldEditedRange: NSRange, editedRange: NSRange, changeInLength: Int
  ) -> Int {
  ...
  private static func updatedUpperBound(
    of range: NSRange, oldEditedRange: NSRange, editedRange: NSRange, changeInLength: Int
  ) -> Int {
