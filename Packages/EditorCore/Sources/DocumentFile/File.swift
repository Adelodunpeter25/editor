...
public struct File: Equatable, Sendable {
  ...
  public nonisolated static let metadataResourceKeys: Set<URLResourceKey> = [
    .isDirectoryKey, .isHiddenKey, .isWritableKey, .isAliasFileKey, .contentTypeKey,
  ]
  ...
  public init(at fileURL: URL) throws {
    ...
    self.kind =
      if self.isAlias, (try? URL(resolvingAliasFileAt: fileURL).isDirectory) == true {
        .folder
      } else {
    ...
    self.tags =
      (try? fileURL.extendedAttribute(for: ExtendedFileAttributeName.userTags))
