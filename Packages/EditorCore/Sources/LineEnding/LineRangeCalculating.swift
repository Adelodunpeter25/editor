...
extension LineRangeCalculating {
  ...
  public func lineNumber(at characterIndex: Int) -> Int {
  ...
  public func lineStartIndex(at characterIndex: Int) -> Int {
  ...
  public func lineRange(at characterIndex: Int) -> NSRange {

    assert(characterIndex <= self.length)

    (self as? any LazyLineEndingCaching)?.ensureLineEndings(
      upTo: characterIndex, needsNextEnd: true)
  ...
  public func lineContentsRange(for range: NSRange) -> NSRange {
    ...
    (self as? any LazyLineEndingCaching)?.ensureLineEndings(
      upTo: range.upperBound, needsNextEnd: true)
...
extension RandomAccessCollection where Element == ValueRange<LineEnding>, Index == Int {
  ...
  func lineBounds(for range: NSRange) -> (start: Int, end: ValueRange<LineEnding>?) {
    ...
    let start =
      (startPassedIndex > self.startIndex)
      ? self[self.index(before: startPassedIndex)].upperBound : 0
    ...
    if let startPassed, range.upperBound <= startPassed.upperBound {
      ...
      let index = try! self.partitioningIndex(in: startPassedIndex..<self.endIndex) {
