...
extension String {
  ...
  public func range(in range: FuzzyRange) -> NSRange? {
  ...
  public func rangeForLine(in lineRange: FuzzyRange, includingLineEnding: Bool = true) -> NSRange? {
    ...
    let newLocation =
      (lineRange.location > 0) ? lineRange.location - 1 : (count + lineRange.location)  // 1-based to 0-based
    let newLength: Int =
  ...
  public func fuzzyLocation(line: Int, column: Int = 0) throws -> Int {
    ...
    guard
      let rangeInLine = (self as NSString).substring(with: lineRange).range(in: fuzzyColumnRange)
    else {
