...
extension StringProtocol {
  ...
  public func normalizing(in form: UnicodeNormalizationForm) -> String {
...
extension String {
  ...
  var precomposedStringWithHFSPlusMapping: String {
    ...
    return regex.matches(in: self, range: NSRange(..<self.utf16.count))
      ...
      .reduce(into: mutable) {
        $0.replaceCharacters(
          in: $1, with: $0.substring(with: $1).precomposedStringWithCanonicalMapping)
      } as String
  }
  ...
  var decomposedStringWithHFSPlusMapping: String {
    ...
    guard CFStringGetFileSystemRepresentation(self as CFString, &buffer, length) else {
      return self
    }
