...
extension Sequence where Element == QuotePairRule {
  ...
  public var distinctForMatching: [QuotePairRule] {

    self.reduce(into: []) { result, rule in
      if let index = result.firstIndex(where: {
        $0.pair == rule.pair && $0.escapeCharacter == rule.escapeCharacter
      }) {
...
extension String {
  ...
  public func rangeOfQuotePair(at index: Index, candidates: [QuotePairRule]) -> ClosedRange<Index>?
  {
    ...
    return
      ...
      .compactMap { candidate -> (range: ClosedRange<Index>, distance: Int, hasPrefix: Bool)? in
        guard
          let range = self.rangeOfSymbolPair(
            at: index, candidates: [candidate.pair], escapeCharacter: candidate.escapeCharacter)
        else { return nil }
