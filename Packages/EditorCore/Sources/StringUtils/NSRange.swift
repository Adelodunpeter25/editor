...
extension NSRange {
  ...
  public func touches(_ index: Int) -> Bool {
  ...
  public func touches(_ range: NSRange) -> Bool {
  ...
  public func clamped(around target: NSRange, maxLength: Int, headPadding: Int = 64) -> NSRange {
  ...
  public func union(with ranges: [NSRange?]) -> NSRange {
...
extension NSRange {
  ...
  public struct InsertionItem: Equatable, Sendable {
  ...
  public func inserted(items: [Self.InsertionItem]) -> NSRange {
    ...
    let location =
      items
      .prefix {
        (self.isEmpty && $0.forward)
          ? $0.location <= self.lowerBound : $0.location < self.lowerBound
      }
      .map(\.string.length)
    ...
    let length =
      ...
      .filter {
        (self.isEmpty || !$0.forward)
          ? self.lowerBound < $0.location : self.lowerBound <= $0.location
      }
      .filter {
        (self.isEmpty || $0.forward)
          ? $0.location < self.upperBound : $0.location <= self.upperBound
      }
      .map(\.string.length)
      .reduce(self.length, +)
  ...
  /// - Returns: A new range that the receiver moved.
  public func removed(ranges: [NSRange]) -> NSRange {
...
extension IndexSet {
  ...
  public init(integersIn ranges: [NSRange]) {
