...
extension NSString {
  ...
  /// The whole range, expressed as an NSRange.
  public final var range: NSRange {
  ...
  public final func index(before location: Int) -> Int {
    ...
    let isCRLF =
      (index > 0 && self.character(at: index) == 0xA && self.character(at: index - 1) == 0xD)
  ...
  public final func index(after location: Int) -> Int {
  ...
  public final func ranges(
    of searchString: String, options: NSString.CompareOptions = .literal,
    ...
    while location <= searchRange.upperBound {
      let range = self.range(
        of: searchString, options: options, range: NSRange(location..<searchRange.upperBound))
      ...
      location =
        if range.isEmpty {
          range.upperBound < searchRange.upperBound
            ? self.index(after: range.upperBound) : NSNotFound
        } else {
          range.upperBound
        }
    }
  ...
  /// - Returns: The line range containing the given location.
  public final func lineRange(at location: Int) -> NSRange {
  ...
  public final func lineContentsRange(at location: Int) -> NSRange {
  ...
  public final func lineContentsRange(for range: NSRange) -> NSRange {
  ...
  public final func lineStartIndex(at index: Int) -> Int {
  ...
  public final func lineContentsEndIndex(at index: Int) -> Int {
  ...
  public final func lineRanges(for ranges: [NSRange], includingLastEmptyLine: Bool = false)
    -> [NSRange]
  {
    ...
    for scope in scopes {
      self.enumerateSubstrings(in: scope, options: [.byLines, .substringNotRequired]) {
  ...
  public final func lineNumber(at location: Int) -> Int {
    ...
    while index < location {
      self.getLineStart(
        nil, end: &index, contentsEnd: nil, for: NSRange(location: index, length: 0))
      count += 1
    }
  ...
  public final func safeEndingRange(for range: NSRange) -> NSRange {
  ...
  public final func rangeOfCharacter(until set: CharacterSet, at index: Int, range: NSRange? = nil)
    -> NSRange
  {
    ...
    let lowerDelimiterRange = self.rangeOfCharacter(
      from: set, options: .backwards, range: NSRange(range.lowerBound..<index))
    let lowerBound =
      !lowerDelimiterRange.isNotFound ? lowerDelimiterRange.upperBound : range.lowerBound

    let upperDelimiterRange = self.rangeOfCharacter(
      from: set, range: NSRange(index..<range.upperBound))
    let upperBound =
      !upperDelimiterRange.isNotFound ? upperDelimiterRange.lowerBound : range.upperBound
  ...
  public final func lowerBoundOfComposedCharacterSequence(_ index: Int, offsetBy offset: Int) -> Int
  {
    ...
    let options: EnumerationOptions = [
      .byComposedCharacterSequences, .substringNotRequired, .reverse,
    ]
