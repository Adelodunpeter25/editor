...
extension String {
  ...
  public func smartIndent(
    style: IndentStyle, indentWidth: Int, tokens: [IndentToken] = [], in selectedRanges: [NSRange]
  ) -> EditingContext? {
    ...
    for selectedRange in selectedRanges {
      ...
      let baseIndent =
        if let indentRange = self.rangeOfIndent(at: lastLocation) {
      ...
      let candidates = tokens.filter {
        self.matches(token: $0.begin, before: lastLocation, ignoreCase: $0.ignoreCase)
      }
      if !candidates.isEmpty {
        ...
        if candidates.contains(where: {
          ...
          return self.matches(
            token: end, after: selectedRange.upperBound, ignoreCase: $0.ignoreCase)
        }) {
    ...
    return EditingContext(
      strings: indents, ranges: replacementRanges, selectedRanges: newSelectedRanges)
  }
  ...
  public func smartOutdentLevel(
    with string: String, indentWidth: Int, tokens: [IndentToken] = [], in range: NSRange
  ) -> Int {
...
extension NSString {
    ...
    let foundRange =
      if token.first?.isLetter == true {
        self.range(
          ...
          options: options.union(.regularExpression), range: range)
      } else {
  ...
  func matches(token: String, after location: Int, ignoreCase: Bool = false) -> Bool {
    ...
    let options: NSString.CompareOptions = ignoreCase ? .caseInsensitive : []
    let foundRange =
      if token.last?.isLetter == true {
        self.range(
          of: "^\(NSRegularExpression.escapedPattern(for: token))(?![A-Za-z0-9_])",
          options: options.union(.regularExpression), range: range)
      } else {
