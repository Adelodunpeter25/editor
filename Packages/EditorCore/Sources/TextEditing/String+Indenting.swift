...
extension String {
  ...
  public func indent(style: IndentStyle, indentWidth: Int, in selectedRanges: [NSRange])
    -> EditingContext
  {
    ...
    let newSelectedRanges = selectedRanges.map { selectedRange -> NSRange in
      ...
      return NSRange(
        location: selectedRange.location + shift * indentLength,
  ...
  public func outdent(style: IndentStyle, indentWidth: Int, in selectedRanges: [NSRange])
    -> EditingContext?
  {
    ...
    let newSelectedRanges = selectedRanges.map { selectedRange -> NSRange in
      ...
      return NSRange(
        location: selectedRange.location - offset,
        length: selectedRange.length - lengthDiff)
    }
  ...
  ///   - selectedRanges: The selection in the editor.
  /// - Returns: An `EditingContext`, or `nil` if no changes were necessary.
  public func convertIndentation(
    to style: IndentStyle, indentWidth: Int, in selectedRanges: [NSRange]
  ) -> EditingContext? {
...
extension String {
  ...
  public var detectedIndentStyle: IndentStyle? {
  ...
  public func rangeOfIndent(at location: Int) -> NSRange? {

    let lineRange = (self as NSString).lineRange(at: location)
    let range = (self as NSString).range(
      of: "^[ \\t]++", options: .regularExpression, range: lineRange)
  ...
  public func rangeOfIndent(at index: String.Index) -> Range<String.Index>? {
    ...
    let lower = self.index(
      lineRange.lowerBound, offsetBy: match.range.lowerBound.utf16Offset(in: self[lineRange]))
    let upper = self.index(
      lineRange.lowerBound, offsetBy: match.range.upperBound.utf16Offset(in: self[lineRange]))
    return lower..<upper
  }
  ...
  public func rangeForSoftTabDeletion(in range: NSRange, tabWidth: Int) -> NSRange? {
    ...
    guard
      (self as NSString).range(of: "^ ++$", options: .regularExpression, range: forwardRange).length
        > 1
    else { return nil }
    ...
    guard (self as NSString).substring(with: targetRange).allSatisfy({ $0 == " " }) else {
      return nil
    }
  ...
  public func softTab(at location: Int, tabWidth: Int) -> String {
