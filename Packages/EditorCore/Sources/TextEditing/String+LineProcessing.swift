...
extension String {
  ...
  public func moveLineUp(in ranges: [NSRange]) -> EditingContext? {
    ...
    for lineRange in lineRanges {
      ...
      for (index, selectedRange) in ranges.enumerated() {
        ...
        if let shiftedRange {
          movedSelectedRanges[index] =
            movedSelectedRanges[index]?.union(shiftedRange) ?? shiftedRange
        }
      }
    }
    let selectedRanges = movedSelectedRanges.compactMap { $0 }.uniqued.sorted(
      using: KeyPathComparator(\.location))
    let replacementString = string.substring(with: replacementRange)

    return EditingContext(
      strings: [replacementString], ranges: [replacementRange], selectedRanges: selectedRanges)
  }
  ...
  /// - Parameter ranges: The selection ranges whose intersecting lines should be moved.
  /// - Returns: An `EditingContext`, or `nil` if no move is possible.
  public func moveLineDown(in ranges: [NSRange]) -> EditingContext? {
    ...
    for lineRange in lineRanges.reversed() {
      ...
      let offset =
        (lineString.last?.isNewline == true)
  ...
  public func deleteDuplicateLine(in ranges: [NSRange]) -> EditingContext? {

    let string = self as NSString
    let lineContentsRanges =
      ranges
  ...
  public func duplicateLine(in ranges: [NSRange], lineEnding: Character) -> EditingContext? {
    ...
    return EditingContext(
      strings: replacementStrings, ranges: replacementRanges, selectedRanges: selectedRanges)
  }
  ...
  public func deleteLine(in ranges: [NSRange]) -> EditingContext? {
    ...
    return EditingContext(
      strings: replacementStrings, ranges: lineRanges, selectedRanges: selectedRanges)
  }
  ...
  public func joinLines(in ranges: [NSRange]) -> EditingContext {
    ...
    for (range, replacementString) in zip(ranges, replacementStrings) {
      selectedRanges.append(
        NSRange(location: range.location + offset, length: replacementString.length))
      offset += replacementString.length - range.length
    }

    return EditingContext(
      strings: replacementStrings, ranges: ranges, selectedRanges: selectedRanges)
  }
  ...
  /// - Returns: An `EditingContext`.
  public func joinLines(after ranges: [NSRange]) -> EditingContext {
    ...
    let replacementRanges =
      ...
      .map {
        (self as NSString).range(
          of: #"\s*\R\s*"#, options: .regularExpression, range: NSRange($0.lowerBound..<self.length)
        )
      }
...
extension String {
  ...
  public func sortLinesAscending(in range: NSRange, baseLineEnding: String? = nil)
    -> EditingContext?
  {
  ...
  ///   - baseLineEnding: The line ending to add when a line without one moves before a line-ending slot.
  /// - Returns: An `EditingContext`, or `nil` if only a single line is present.
  public func reverseLines(in range: NSRange, baseLineEnding: String? = nil) -> EditingContext? {
  ...
  ///   - baseLineEnding: The line ending to add when a line without one moves before a line-ending slot.
  /// - Returns: An `EditingContext`, or `nil` if only a single line is present.
  public func shuffleLines(in range: NSRange, baseLineEnding: String? = nil) -> EditingContext? {
  ...
  private func sortLines(
    in range: NSRange, baseLineEnding: String?, predicate: ([LogicalLine]) -> [LogicalLine]
  ) -> EditingContext? {
    ...
    let newString = predicate(lines)
      .joined(
        baseLineEnding: baseLineEnding, includingTrailingLineEnding: includesTrailingLineEnding)
    ...
    return EditingContext(
      strings: [newString], ranges: [lineRange], selectedRanges: [selectedRange])
  }
}
...
extension String {
  ...
  public func trimTrailingWhitespace(
    ignoringEmptyLines: Bool, keepingEditingPoint: Bool = false, in editingRanges: [NSRange]
  ) -> EditingContext? {
    ...
    return EditingContext(
      strings: replacementStrings, ranges: trimmingRanges, selectedRanges: selectedRanges)
  }
}
