...
extension String {
  ...
  public func commentOut(
    types: CommentTypes, delimiters: any CommentDelimiters, appendsSpacer: Bool,
    in selectedRanges: [NSRange], at location: CommentOutLocation
  ) -> EditingContext? {
    ...
    let items: [NSRange.InsertionItem] = {
      if types.contains(.inline), let delimiter = delimiters.inlineDelimiters.first {
        return self.inlineCommentOut(
          delimiter: delimiter, appendsSpacer: appendsSpacer, ranges: selectedRanges, at: location)
      }
      if types.contains(.block), let delimiters = delimiters.blockDelimiters.first {
        return self.blockCommentOut(
          delimiters: delimiters.pair, appendsSpacer: appendsSpacer, ranges: selectedRanges,
          at: location)
      }
      return []
    }()
    ...
    return EditingContext(
      strings: newStrings, ranges: replacementRanges, selectedRanges: newSelectedRanges)
  }
  ...
  public func uncomment(
    delimiters: any CommentDelimiters, appendsSpacer: Bool, in selectedRanges: [NSRange]
  ) -> EditingContext? {

    guard !delimiters.isEmpty else { return nil }

    let deletionRanges: [NSRange] =
      delimiters.blockDelimiters.lazy
      .compactMap { delimiters in
        let targetRanges = selectedRanges.map { $0.isEmpty ? self.lineContentsRange(for: $0) : $0 }
          .uniqued
        return self.rangesOfBlockDelimiters(
          delimiters, appendsSpacer: appendsSpacer, ranges: targetRanges)
      }.first
      ?? delimiters.inlineDelimiters.lazy
      .compactMap { delimiter in
        let targetRanges = selectedRanges.map(self.lineContentsRange(for:)).uniqued
        return self.rangesOfInlineDelimiter(
          delimiter, appendsSpacer: appendsSpacer, ranges: targetRanges)
      }.first
      ?? []
    ...
    return EditingContext(
      strings: newStrings, ranges: deletionRanges, selectedRanges: newSelectedRanges)
  }
  ...
  public func canUncomment(
    partly: Bool, delimiters: any CommentDelimiters, in selectedRanges: [NSRange]
  ) -> Bool {
    ...
    for delimiters in delimiters.blockDelimiters {
      if let ranges = self.rangesOfBlockDelimiters(
        delimiters, appendsSpacer: false, ranges: targetRanges)
      {
        return partly ? true : (ranges.count == (2 * targetRanges.count))
      }
    }

    for delimiter in delimiters.inlineDelimiters {
      if let ranges = self.rangesOfInlineDelimiter(
        delimiter, appendsSpacer: false, ranges: targetRanges)
      {
...
extension String {
  ...
  func inlineCommentOut(
    delimiter: String, appendsSpacer: Bool = false, ranges: [NSRange],
    at location: CommentOutLocation
  ) -> [NSRange.InsertionItem] {
    ...
    let spacer = appendsSpacer ? " " : ""

    return locations.map {
      NSRange.InsertionItem(string: delimiter + spacer, location: $0, forward: true)
    }
  }
  ...
  ///   - location: The location type to insert comment delimiters.
  /// - Returns: Items that contain editing information to insert comment delimiters.
  func blockCommentOut(
    delimiters: Pair<String>, appendsSpacer: Bool = false, ranges: [NSRange],
    at location: CommentOutLocation
  ) -> [NSRange.InsertionItem] {
    ...
    let targetRanges =
      ...
      case .afterIndent(let tabWidth):
        ranges
          ...
          .map {
            let lowerBound =
              self.minimumCommonIndentationLocations(for: [$0], tabWidth: tabWidth).first
              ?? $0.lowerBound
    ...
    return targetRanges.flatMap {
      [
        NSRange.InsertionItem(
          string: delimiters.begin + spacer, location: $0.lowerBound, forward: true),
        NSRange.InsertionItem(
          string: spacer + delimiters.end, location: $0.upperBound, forward: false),
      ]
    }
  }
  ...
  func rangesOfInlineDelimiter(_ delimiter: String, appendsSpacer: Bool, ranges: [NSRange])
    -> [NSRange]?
  {
  ...
  ///   - ranges: The ranges where to find.
  /// - Returns: Ranges where delimiters and spacers are, or `nil` when no delimiters was found.
  func rangesOfBlockDelimiters(
    _ delimiters: BlockCommentDelimiter, appendsSpacer: Bool, ranges: [NSRange]
  ) -> [NSRange]? {
    ...
    let spacerPattern = appendsSpacer ? " ?" : ""
    let pattern =
      "\\A[ \t]*(\(beginPattern + spacerPattern)).*?(\(spacerPattern + endPattern))[ \t]*\\Z"
  ...
  private func isValidNestableBlock(_ delimiters: BlockCommentDelimiter, in bounds: Pair<NSRange>)
    -> Bool
  {
    ...
    let beginTokens = (self as NSString).ranges(of: delimiters.begin, range: innerRange).map {
      (isBegin: true, range: $0)
    }
    let endTokens = (self as NSString).ranges(of: delimiters.end, range: innerRange).map {
      (isBegin: false, range: $0)
    }
