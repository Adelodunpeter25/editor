...
extension NSString {
  ...
  public func charactersIncompatible(with encoding: String.Encoding) throws -> [ValueRange<
...
extension NSString {
  ...
  /// - Throws: `CancellationError` if the task is cancelled.
  fileprivate func findIncompatibles(in range: NSRange, encoding: String.Encoding) throws
    -> [ValueRange<IncompatibleCharacter>]
  {
    ...
    if let converted {
      ...
      if self.isSingleComposedCharacter(in: range) {
        let incompatible = IncompatibleCharacter(
          character: originalChunk.first, converted: converted)
  ...
  fileprivate func roundTrip(in range: NSRange, encoding: String.Encoding) -> String? {
    ...
    while capacity <= maxCapacity {
      ...
      let ok = buffer.withUnsafeMutableBytes { raw -> Bool in
        ...
        return self.getBytes(
          base, maxLength: raw.count, usedLength: &usedLength, encoding: encoding.rawValue,
          options: .allowLossy, range: range, remaining: &remaining)
      }
  ...
  fileprivate func isSingleComposedCharacter(in range: NSRange) -> Bool {
  ...
  fileprivate func splitRangeOnComposedBoundary(in range: NSRange) -> (NSRange, NSRange) {
    ...
    return (
      self.rangeOfComposedCharacterSequences(for: left),
