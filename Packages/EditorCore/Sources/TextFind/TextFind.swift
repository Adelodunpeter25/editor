...
public struct TextFind: Equatable, Sendable {
  ...
  public init(
    for string: String, findString: String, mode: TextFind.Mode, inSelection: Bool = false,
    selectedRanges: [NSRange] = [NSRange()]
  ) throws {
  ...
  public func find(
    in matches: [NSRange], forward: Bool, includingSelection: Bool = false, wraps: Bool
  ) -> (range: NSRange, wrapped: Bool)? {
  ...
  public func replace(with replacementString: String) -> ReplacementItem? {
    ...
    case .textual(let options, _):
      let matchedRange = (string as NSString).range(
        of: self.findString, options: options, range: selectedRange)
    ...
    case .regularExpression:
      ...
      guard
        let match = regex.firstMatch(
          in: string, options: [.withTransparentBounds, .withoutAnchoringBounds],
          range: selectedRange)
      else { return nil }

      let template = self.replacementString(from: replacementString)
      let replacedString = regex.replacementString(
  ...
  public func replaceAll(
    ...
    using block: @escaping (_ range: NSRange, _ count: Int, _ stop: inout Bool) -> Void
  ) -> (replacementItems: [ReplacementItem], selectedRanges: [NSRange]?) {
    ...
    for scopeRange in self.scopeRanges {
      let scopeString = NSMutableString(
        string: (self.string as NSString).substring(with: scopeRange))
      var ioStop = false

      // replace string
      switch self.mode {
      case .textual(let options, let fullWord) where !fullWord:
        // replace at once for performance
        let count = scopeString.replaceOccurrences(
          of: self.findString, with: replacementString, options: options, range: scopeString.range)
      ...
      default:
        ...
        self.enumerateMatches(in: scopeRange) { matchedRange, match, stop in
          let replacedString: String =
            if let match, let regex = match.regularExpression {
      ...
      if scopeString.compare(originalString, options: .literal) != .orderedSame {
        replacementItems.append(
          ReplacementItem(value: scopeString.copy() as! String, range: scopeRange))
      }
  ...
  private func checkFullWord(in range: NSRange) -> Bool {
    ...
    return fullWordChecker.firstMatch(
      in: self.string, options: .withTransparentBounds, range: range) != nil
  }
  ...
  private func enumerateMatches(
    in range: NSRange,
    using block: (_ matchedRange: NSRange, _ match: NSTextCheckingResult?, _ stop: inout Bool) ->
      Void
  ) {
  ...
  ///   - block: The block that enumerates the matches.
  private func enumerateTextualMatches(
    in range: NSRange, options: String.CompareOptions, fullWord: Bool,
    using block: (_ matchedRange: NSRange, _ match: NSTextCheckingResult?, _ stop: inout Bool) ->
      Void
  ) {
  ...
  ///   - range: The range of the string to search.
  ///   - block: The block that enumerates the matches.
  private func enumerateRegularExpressionMatches(
    in range: NSRange,
    using block: (_ matchedRange: NSRange, _ match: NSTextCheckingResult?, _ stop: inout Bool) ->
      Void
  ) {
