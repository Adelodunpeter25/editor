...
public struct MultipleReplace: Equatable, Sendable, Codable {
  ...
  public struct Replacement: Equatable, Sendable {
    ...
    public var isEmpty: Bool {
      self.findString.isEmpty && self.replacementString.isEmpty && self.description == nil
    }

    public init(
      findString: String = "", replacementString: String = "", usesRegularExpression: Bool = false,
      ignoresCase: Bool = false, description: String? = nil, isEnabled: Bool = true
    ) {
  ...
  public struct Settings: Equatable, Sendable {
    ...
    public init(
      ...
      regexOptions: NSRegularExpression.Options = [.anchorsMatchLines],
      matchesFullWord: Bool = false, unescapesReplacementString: Bool = true
    ) {
...
extension MultipleReplace.Replacement {
  ...
  public static func == (lhs: Self, rhs: Self) -> Bool {
    ...
    (lhs.findString as NSString).isEqual(to: rhs.findString)
      ...
      && Self.isLiteralEqual(lhs.description, to: rhs.description) && lhs.isEnabled == rhs.isEnabled
  }
...
extension MultipleReplace {
  ...
  public func find(
    string: String, ranges: [NSRange], inSelection: Bool, progress: FindProgress? = nil
  ) throws -> [NSRange] {
    ...
    for replacement in self.replacements where replacement.isEnabled {
      ...
      guard
        let textFind = try? TextFind(
          for: string, findString: replacement.findString, mode: mode, inSelection: inSelection,
  ...
  public func replace(
    ...
    for replacement in self.replacements where replacement.isEnabled {
      ...
      guard
        let textFind = try? TextFind(
          for: result.string, findString: replacement.findString, mode: mode,
          inSelection: inSelection, selectedRanges: findRanges)
      else { continue }

      // process replacement
      let (replacementItems, selectedRanges) = textFind.replaceAll(
...
extension MultipleReplace.Replacement {
  ...
  fileprivate func mode(settings: MultipleReplace.Settings) -> TextFind.Mode {

    if self.usesRegularExpression {
      let options = settings.regexOptions.union(self.ignoresCase ? [.caseInsensitive] : [])

      return .regularExpression(
        options: options, unescapesReplacement: settings.unescapesReplacementString)
