...
/// Resolves the SwiftPM resource bundle (`Editor_Editor.bundle`) holding the grammar JSON.
///
...
private enum GrammarBundle {
  static let bundle: Bundle? = {
    if let res = Bundle.main.resourceURL {
      let url = res.appendingPathComponent("Editor_Editor.bundle")
...
final class TMRule: Decodable {
  ...
  private enum CodingKeys: String, CodingKey {
    case name, contentName, match, begin, end, include, captures, beginCaptures, endCaptures,
      patterns
  }
...
enum TMTheme {
  ...
  static func color(for scope: String) -> NSColor? {
    ...
    for (prefix, color) in map where prefix.count > bestLen {
      if scope == prefix || scope.hasPrefix(prefix + ".") {
        best = color
...
final class TextMateHighlighter {
  ...
  static var availableLanguages: [String] {
    guard
      let urls = GrammarBundle.bundle?.urls(
        forResourcesWithExtension: "json", subdirectory: "Grammars")
    else { return [] }
  ...
  private static func load(language: String) -> TextMateHighlighter? {
    ...
    guard
      let url = GrammarBundle.bundle?.url(
        forResource: language, withExtension: "json", subdirectory: "Grammars"),
  ...
  private static func substituteBackrefs(
    _ pattern: String, match: NSTextCheckingResult, in ns: NSString
  ) -> String {
    var out = ""
    ...
    for rule in rules {
      if let include = rule.include {
        ...
        if include == "$self" || include == "$base" {
          ...
          let target = grammar.repository?[String(include.dropFirst())]
        {
          if target.match != nil || target.begin != nil {
            result.append(target)
          } else if let nested = target.patterns {
            result += expand(nested, visited: next)
          }
        }
  ...
  private func tokenizeLine(
    _ lineRange: NSRange, text: String, ns: NSString,
    stack: inout [Frame], out: inout [(NSRange, NSColor)]
  ) {
    let lineEnd = lineRange.location + lineRange.length
    var pos = lineRange.location
    var nextMatch = [NSTextCheckingResult?](repeating: nil, count: ruleCount)
    var hasCachedMatch = [Bool](repeating: false, count: ruleCount)
    ...
    while pos <= lineEnd {
      ...
      for rule in top.patterns {
          ...
          if let cached = nextMatch[rid] {
            if cached.range.location >= pos {
              ...
              match = regex.firstMatch(
                in: text, options: Self.matchOptions,
        ...
        if let match, match.range.location < bestStart {
          bestStart = match.range.location
          best = match
          bestRule = rule
        }
      }

      let endMatch = top.endRegex?.firstMatch(
        in: text, options: Self.matchOptions,
        range: NSRange(location: pos, length: lineEnd - pos))
      ...
      if let end = endMatch, best == nil || end.range.location <= bestStart {
        ...
        if let rule = top.rule {
          if let name = rule.name, let color = TMTheme.color(for: name) {
            out.append((end.range, color))
          }
        ...
        if rule.begin != nil, let endRegex = resolvedEnd(rule, match: match, ns: ns) {
          ...
          if zeroWidth, let last = lastZeroWidthPush, last == key {
            ...
            let contentColor =
              rule.contentName.flatMap { TMTheme.color(for: $0) }
              ?? rule.name.flatMap { TMTheme.color(for: $0) } ?? top.contentColor
            stack.append(
              Frame(
                rule: rule, endRegex: endRegex,
  ...
  private func applyCaptures(
    _ captures: [String: TMCapture]?, match: NSTextCheckingResult,
