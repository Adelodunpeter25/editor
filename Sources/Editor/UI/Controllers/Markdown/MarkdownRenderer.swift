...
enum MarkdownRenderer {
  ...
  private static func italic(_ f: NSFont) -> NSFont {
  ...
  static func render(_ source: String, baseURL: URL?) -> NSAttributedString {
    ...
    while i < lines.count {
      ...
      if let (level, text) = heading(trimmed) {
        out.append(headingBlock(text, level: level, baseURL: baseURL))
        i += 1
        continue
      }
  ...
  private static func isTableSeparator(_ line: String) -> Bool {
    ...
    return !cells.isEmpty
      && cells.allSatisfy { Set($0.trimmingCharacters(in: .whitespaces)).isSubset(of: [":", "-"]) }
  }
  ...
  private static func headingBlock(_ text: String, level: Int, baseURL: URL?) -> NSAttributedString
  {
    ...
    let base: [NSAttributedString.Key: Any] = [
      .font: headingFont(level), .foregroundColor: headingColor, .paragraphStyle: para,
    ]
  ...
  private static func listBlock(_ items: [(marker: String, text: String)], baseURL: URL?)
    -> NSAttributedString
  {
  ...
  private static func quoteBlock(_ text: String, baseURL: URL?) -> NSAttributedString {
    ...
    let base: [NSAttributedString.Key: Any] = [
      .font: italic(bodyFont), .foregroundColor: quoteColor, .paragraphStyle: para,
    ]
    let r = NSMutableAttributedString(attributedString: inline(text, base: base, baseURL: baseURL))
    r.append(NSAttributedString(string: "\n", attributes: base))
    return r
    ...
    for (r, row) in rows.enumerated() {
      ...
      for c in 0..<columns {
        let cellBlock = NSTextTableBlock(
          table: textTable, startingRow: r, rowSpan: 1, startingColumn: c, columnSpan: 1)
        ...
        let base: [NSAttributedString.Key: Any] = [
          .font: isHeader ? bold(bodyFont) : bodyFont, .foregroundColor: textColor,
          .paragraphStyle: para,
        ]
  ...
  private static func inline(_ text: String, base: [NSAttributedString.Key: Any], baseURL: URL?)
    -> NSAttributedString
  {
  ...
  private static func imageAttachment(_ url: URL, baseURL: URL?) -> NSTextAttachment? {
    ...
    if image.size.width > maxWidth {
      image.size = NSSize(
        width: maxWidth, height: image.size.height * (maxWidth / image.size.width))
    }
