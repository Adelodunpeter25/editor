...
final class ImageViewController: NSViewController, SourceEditing {
  ...
  override func loadView() {
    ...
    imageView.frame = NSRect(
      origin: .zero, size: nativeSize == .zero ? NSSize(width: 240, height: 140) : nativeSize)
  ...
  private func addSourceViewAndToggle(into root: NSView, bottomBar: NSStackView) {
    ...
    let toggle = PointerSegmentedControl(
      labels: ["Image", "Source"], trackingMode: .selectOne, target: self,
  ...
  private func fitMagnification() -> CGFloat {
    ...
    guard nativeSize.width > 0, nativeSize.height > 0, viewport.width > 0, viewport.height > 0
    else { return 1 }
