...
enum NewProject {
  static func present(model: AppModel) {
    ...
    let gitCheck = NSButton(
      checkboxWithTitle: "Initialize a Git repository", target: nil, action: nil)
    gitCheck.state = .off
    ...
    NSLayoutConstraint.activate([
      ...
      gitCheck.trailingAnchor.constraint(
        lessThanOrEqualTo: accessory.trailingAnchor, constant: -20),
    ])
    ...
    if fm.fileExists(atPath: path, isDirectory: &isDir) {
      ...
      if isDir.boolValue {
        ...
        error(
          ...
          "A file already exists at “\((path as NSString).lastPathComponent)”.")
      }
      return
    }
    do {
