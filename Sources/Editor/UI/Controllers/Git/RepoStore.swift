...
final class RepoStore: ObservableObject {
  ...
  private func pollTree() {
    ...
    DispatchQueue.global().async { [weak self] in
      ...
      var hasher = Hasher()
      for f in files {
        hasher.combine(f.path)
        hasher.combine(f.status.rawValue)
        hasher.combine(f.isDir)
      }
  ...
  private func pollChanges() {
    ...
    DispatchQueue.global().async { [weak self] in
      ...
      let sig =
        (g.staged + g.unstaged).map { "\($0.path)|\($0.status.rawValue)" }.joined(separator: "\n")
  ...
  func commit(_ msg: String, all: Bool) {
    act {
      if all { Git.stageAll(self.repo) }
      Git.commit(self.repo, msg)
    }
  }
  func commitPush(_ msg: String, all: Bool) {
    act {
      if all { Git.stageAll(self.repo) }
      Git.commit(self.repo, msg)
      Git.push(self.repo)
