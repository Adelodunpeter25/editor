  ...
  NSTableViewDelegate, NSMenuDelegate
{
  ...
  init(
    store: RepoStore, onOpenDiff: @escaping (String) -> Void, onOpenFile: @escaping (String) -> Void
  ) {
    self.store = store
  ...
  private func rebuild() {
    ...
    if !store.staged.isEmpty {
      rows.append(
        .header(
          title: "STAGED CHANGES", count: store.staged.count,
          actions: [
    ...
    if !store.unstaged.isEmpty {
      ...
      if store.stashCount > 0 {
        changeActions.append(
          ("tray.and.arrow.up", "Unstash (pop latest)", { [weak self] in self?.store.unstash() }))
      }
      changeActions.append(
        (
          "arrow.uturn.backward", "Discard all changes",
          { [weak self] in self?.confirmDiscardAll() }
        ))
  ...
  private func sectionHeader(_ title: String, _ count: Int, actions: [(String, String, () -> Void)])
    -> NSView
  {
  ...
  private func iconButton(_ symbol: String, _ help: String, _ action: @escaping () -> Void)
    -> NSButton
  {
  ...
  @objc private func commitMenuTapped() {
    ...
    let menu = NSMenu()
    let c = menu.addItem(withTitle: "Commit", action: #selector(menuCommit), keyEquivalent: "")
    c.target = self
    let p = menu.addItem(
      withTitle: "Commit & Push", action: #selector(menuCommitPush), keyEquivalent: "")
    p.target = self
  ...
  private func confirmDiscardAll() {
    ...
    alert.informativeText =
      "Resets tracked files to HEAD and deletes untracked files. This cannot be undone."
  ...
  {
    switch items[row] {
    case .header(let title, let count, let actions):
      return sectionHeader(title, count, actions: actions)
    case .file(let f, let staged):
