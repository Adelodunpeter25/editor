...
final class CenterViewController: NSViewController, NSSplitViewDelegate {
  ...
  private func render() {
    ...
    if tab.kind == .terminal, !tab.exited {
  ...
  /// Show the centered "Session ended" overlay over a terminal tab whose process has exited.
  private func updateEndedOverlay(for tab: Tab, session: Session) {
    let show = tab.kind == .terminal && tab.exited
  ...
  private func makeContentView(for tab: Tab, session: Session) -> NSView {
    ...
    case .file:
      ...
      if ImageViewController.handles(tab.path) {
        let vc = ImageViewController(
          path: tab.path ?? "", settings: model.settings, onDirty: onDirty)
        addChild(vc)
      ...
      let untitled: EditorViewController.UntitledFile? =
        tab.path == nil
      ...
      return vc.view
    case .diff:
      let vc = DiffViewController(
        repo: session.url, path: tab.path ?? "", commitHash: tab.commitHash,
    ...
    case .search:
      ...
      // sidebar instance's `SearchViewController.current` static reference.
  ...
  /// Restore first-responder to the active tab's content (terminal or editor) — called when the
...
/// A prominent centered card shown over a terminal whose process has exited (you typed `exit`),
/// so it isn't missed: a dimming scrim + a card with an icon, title, one-line next-step text, and the
/// actions — Restart (accent/primary) and Close. The scrim is visual only — clicks outside the card pass
/// through so the dead terminal stays scrollable.
final class SessionEndedOverlay: NSView {
  ...
  func configure(
    isClaude: Bool = false, onRestart: @escaping () -> Void,
    onOpenTerminal: @escaping () -> Void, onClose: @escaping () -> Void
  ) {
    self.onRestart = onRestart
    self.onOpenTerminal = onOpenTerminal
    self.onClose = onClose
    titleLabel.stringValue = "Session ended"
    subtitle.stringValue = "Restart the shell, or close the tab."
