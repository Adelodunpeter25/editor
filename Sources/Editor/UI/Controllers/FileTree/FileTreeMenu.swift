...
extension FileTreeViewController {
  ...
  func contextItems(for node: TreeNode?) -> [NSMenuItem] {
    ...
    let copy = [
      item("Copy Path", #selector(ctxCopyPath)),
      item("Copy Relative Path", #selector(ctxCopyRelative)),
    ]
    ...
    if node.isFolder {
      return [
        ...
        .separator(),
      ] + edit + [.separator()] + reveal + [.separator()] + copy
    }
    return edit + [.separator()] + reveal + [.separator()] + copy + [
      .separator(),
      item("New File", #selector(ctxNewFile)), item("New Folder", #selector(ctxNewFolder)),
    ]
  }
  ...
  @objc private func ctxCopyPath() {
    if let n = menuTargetNode {
      Clipboard.copy((store.repo as NSString).appendingPathComponent(n.id))
    }
  }
