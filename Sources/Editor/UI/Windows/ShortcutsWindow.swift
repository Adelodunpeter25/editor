...
/// The keyboard shortcuts Editor supports, grouped — shown in the shortcuts panel (bottom-bar keyboard icon).
...
final class ShortcutsWindowController: NSWindowController {
  ...
  private func buildContent(in panel: NSPanel) {
    ...
    esc.target = self
    esc.action = #selector(closeWindow)
    root.addSubview(esc)
  ...
  private static func rowView(_ item: Shortcuts.Item) -> NSView {
    ...
    for ch in item.keys {
      ...
      cap.leadingAnchor.constraint(
        equalTo: prev?.trailingAnchor ?? caps.leadingAnchor,
        constant: prev == nil ? 0 : 3
      ).isActive = true
      prev = cap
    }
