...
final class Updates: ObservableObject {
    ...
    func detectBrew() {
        DispatchQueue.global(qos: .utility).async {
            ...
            if brew.contains("/") {
                ...
                managed = list.split(separator: "\n").contains { $0.trimmingCharacters(in: .whitespaces) == "editor" }
            }
    ...
    func installNow(app: AppModel) {
        ...
        let flag = (NSTemporaryDirectory() as NSString).appendingPathComponent("editor-update-\(UUID().uuidString).done")
