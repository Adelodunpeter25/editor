...
final class Updates: ObservableObject {
  ...
  let repo = "Rudra370/editor"
  let caskRef = "Rudra370/tap/editor"
    ...
    DispatchQueue.global(qos: .utility).async {
      ...
      if brew.contains("/") {
        ...
        managed = list.split(separator: "\n").contains {
          $0.trimmingCharacters(in: .whitespaces) == "editor"
        }
      }
  ...
  func installNow(app: AppModel) {
    ...
    let session = app.activeSession ?? app.openRepo(NSHomeDirectory())
    let tab = Tab(kind: .terminal, title: "Update")
    session.addTab(tab)
    let appPath = Bundle.main.bundlePath
    let flag = (NSTemporaryDirectory() as NSString).appendingPathComponent(
      "editor-update-\(UUID().uuidString).done")
    // NONINTERACTIVE + --force so Homebrew never stops for a Y/N; `xattr` clears the new app's quarantine;
    // the flag is written only on full success → Editor then auto-relaunches into the new build.
    let command =
      "NONINTERACTIVE=1 brew update && NONINTERACTIVE=1 brew upgrade --cask --force \(caskRef)"
  ...
  private func upToDateAlert() {
    ...
    a.informativeText = "Editor \(current) is the latest version."
...
final class UpdateBannerView: NSView {
  ...
  init(updates: Updates, model: AppModel) {
    ...
    whatsNew.bezelStyle = .inline
    whatsNew.isBordered = false
    whatsNew.contentTintColor = .white
    whatsNew.target = self
    whatsNew.action = #selector(showNotes)
    ...
    later.bezelStyle = .inline
    later.isBordered = false
    later.contentTintColor = .white
  ...
  private func refresh() {
    ...
    if updates.installing {
      label.stringValue = "Updating Editor…"
      actionButton.title = "Relaunch"
      whatsNew.isHidden = true
    } else {
      label.stringValue = "Editor \(updates.latest ?? "") is available"
  ...
  @objc private func primaryAction() {
    if updates.installing {
      updates.relaunch()
    } else if updates.brewManaged {
      updates.installNow(app: model)
    } else {
      updates.download()
    }
  }
  ...
  @objc private func showNotes() {
    let a = NSAlert()
    a.messageText = "What's new in Editor \(updates.latest ?? "")"
