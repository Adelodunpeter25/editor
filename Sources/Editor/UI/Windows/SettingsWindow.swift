...
final class SettingsWindowController: NSWindowController, NSWindowDelegate {
  ...
  private func buildContent() -> NSView {
    segmented = PointerSegmentedControl(
      labels: ["General", "Formatters"], trackingMode: .selectOne,
  ...
  private func buildGeneralPane() -> NSView {
    ...
    let monitor = checkbox(
      "Show resource usage (memory / CPU) in the title bar", \.showResourceMonitor)
    ...
    stepper.target = self
    stepper.action = #selector(fontChanged)
    ...
    quickTermSeg = PointerSegmentedControl(
      labels: ["Floating", "Centered", "Bottom"],
      trackingMode: .selectOne,
      target: self, action: #selector(quickTermModeChanged))
    quickTermSeg.selectedSegment =
      Settings.QuickTermMode.allCases.firstIndex(of: settings.quickTermMode) ?? 0
    ...
    quickTermRow.orientation = .horizontal
    quickTermRow.spacing = 8
  ...
  private func buildFormattersPane() -> NSView {
    ...
    let rows: [NSView] = Formatter.specs.enumerated().map { idx, spec in
      formatterRow(spec, index: idx)
    }
  ...
  private func formatterRow(_ spec: FormatterSpec, index: Int) -> NSView {
    ...
    let enable = PointerButton(
      checkboxWithTitle: "On", target: self, action: #selector(formatterEnableToggled(_:)))
    enable.tag = index
  ...
  private func refreshFormatterStatus() {
    for (i, spec) in Formatter.specs.enumerated() {
      ...
      fmtStatus[i].textColor =
        installed
        ? NSColor(red: 0.45, green: 0.79, blue: 0.57, alpha: 1)
  ...
  private func checkbox(_ title: String, _ keyPath: ReferenceWritableKeyPath<Settings, Bool>)
    -> NSButton
  {
