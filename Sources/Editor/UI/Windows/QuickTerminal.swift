...
final class QuickTerminalController: NSObject, NSWindowDelegate {
  ...
  private func activeSessionChanged() {
    guard isShown else { return }
    guard model.activeSession != nil else {
      hide()
      return
    }  // last session closed → hide
    ensureList()
    syncChrome()
  }
  ...
  private func ensurePanel() -> NSView {
    ...
    p.delegate = self
    p.setFrameAutosaveName("EditorQuickTerminal")
    let host = NSView()
...
final class QuickTermChip: PointerView {
  ...
  init(
    id: String, number: Int, isActive: Bool, onSelect: @escaping () -> Void,
    ...
    layer?.backgroundColor =
      (isActive ? NSColor(white: 1, alpha: 0.12) : NSColor(white: 1, alpha: 0.04)).cgColor
