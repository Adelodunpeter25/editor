...
final class StatusBarView: NSView {
  ...
  @objc private func indentClicked() {
    guard let ed = ActiveEditor.current else { return }
    popUp(
      ["Tabs", "Spaces: 2", "Spaces: 4", "Spaces: 8"], current: ed.indentStyle,
  ...
  @objc private func langClicked() {
    ...
    let auto = menu.addItem(
      withTitle: "Auto-detect", action: #selector(langPicked(_:)), keyEquivalent: "")
    auto.target = self
    auto.representedObject = ""  // "" = clear override
    menu.addItem(.separator())
    for lang in EditorViewController.availableLanguages() {
      let item = menu.addItem(
        withTitle: lang.name, action: #selector(langPicked(_:)), keyEquivalent: "")
      item.target = self
  ...
  @objc private func branchClicked() {
    ...
    for b in branches {
      let item = menu.addItem(
        withTitle: b, action: b == current ? nil : #selector(checkoutBranch(_:)), keyEquivalent: "")
      item.target = self
    ...
    let create = menu.addItem(
      withTitle: "Create New Branch…", action: #selector(createBranchClicked), keyEquivalent: "")
    create.target = self
    ...
    for b in branches where b != current {
      let di = delMenu.addItem(
        withTitle: b, action: #selector(deleteBranchClicked(_:)), keyEquivalent: "")
      di.target = self
  ...
  @objc private func deleteBranchClicked(_ item: NSMenuItem) {
    ...
    DispatchQueue.global().async {
      ...
      DispatchQueue.main.async {
        ...
        self.runGit(
          { Git.deleteBranch(repo, b, force: !merged).error }, failTitle: "Couldn’t delete “\(b)”")
      }
    }
  }
