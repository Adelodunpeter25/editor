...
// the load-bearing half — don't drop them.

private func installPointerTracking(_ view: NSView) {
  guard view.window != nil else { return }
  if view.trackingAreas.contains(where: { $0.owner === view && $0.options.contains(.cursorUpdate) })
  {
    return
  }
...
class PointerOutlineView: NSOutlineView {
  ...
  override func resetCursorRects() {
    ...
    for i in visible.location..<NSMaxRange(visible) {
