...
final class FindBar: NSView, NSTextFieldDelegate {
  ...
  init(matchCase: Bool, wholeWord: Bool, regex: Bool) {
    ...
    let findRow = NSStackView(views: [
      disclosure, field, caseBtn, wordBtn, regexBtn,
  ...
  func setCount(current: Int, total: Int) {
    countLabel.stringValue =
      total == 0 ? (query.isEmpty ? "" : "No results") : "\(current) of \(total)"
  }
  ...
  @objc private func toggleReplace() {
    ...
    disclosure.image = NSImage(
      systemSymbolName: replaceRow.isHidden ? "chevron.right" : "chevron.down",
