...
// DEV-only self-test harness. The dev build reads /tmp/editor-debug.json on launch for
...
// so a stray /tmp file can never affect a real/release Editor.
enum DebugHarness {
  static func start(model: AppModel) {
    guard Bundle.main.isDev,
      let data = try? Data(contentsOf: URL(fileURLWithPath: "/tmp/editor-debug.json")),
    ...
    if let state = cfg["state"] as? String {
      Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { _ in
        DebugState.capture(to: state, model)
      }
    }
    if let actions = cfg["actions"] as? [String] {
      ...
      for action in actions {
        if action.hasPrefix("wait:") {
          delay += Double(action.dropFirst(5)) ?? 0
          continue
        }
...
enum DebugAction {
  static func run(_ action: String, _ model: AppModel) {
    ...
    case "activateTab":
      if let s = model.activeSession, let i = Int(arg), s.tabs.indices.contains(i) {
        s.activate(s.tabs[i].id)
      }
    case "renderAttentionMenu":
      AttentionMenu.debugRender(to: arg.isEmpty ? "/tmp/attention-menu.png" : arg)
    ...
    case "quickMode":
      model.settings.quickTermMode = Settings.QuickTermMode(rawValue: arg) ?? .floating
    case "quickSend":
      if let id = QuickTerminalController.current?.debugActiveID {
        TerminalStore.shared.send(id, arg)
      }
    ...
    case "editorLineColors":  // from-to → dump applied fg colour per line (highlighter-desync diagnosis)
      ...
      if p.count == 2, let lines = ActiveEditor.current?.debugLineColors(p[0], p[1]) {
        try? lines.joined(separator: "\n").write(
          toFile: "/tmp/editor-colordump.txt", atomically: true, encoding: .utf8)
      }
    case "editorColorRuns":  // comma-separated line numbers → per-char colour runs (string-open diagnosis)
      ...
      let dump = lines.map { "L\($0): " + (ActiveEditor.current?.debugColorRuns($0) ?? "?") }
        .joined(separator: "\n")
      try? dump.write(toFile: "/tmp/editor-colorruns.txt", atomically: true, encoding: .utf8)
    case "showShortcuts": ShortcutsWindowController.shared.show()
    case "renderShortcuts":
      ShortcutsWindowController.shared.debugRender(to: arg.isEmpty ? "/tmp/shortcuts.png" : arg)
    case "unsavedResp":  // canned answer for the close/quit guard (modal can't be clicked in harness)
      switch arg {
      case "save": UnsavedGuard.debugResponse = .save
    ...
    case "closeTabGuarded":  // exercises the real guarded close path (⌘W / close button)
      if let s = model.activeSession, let t = s.activeTab, UnsavedGuard.confirmClose(t) {
        s.closeTab(t.id)
      }
    ...
    case "sendText":
      if let s = model.activeSession { TerminalStore.shared.send(s.activeTabID, arg) }
    case "sendEnter":
      if let s = model.activeSession { TerminalStore.shared.send(s.activeTabID, "\r") }
    case "scroll":
      ...
      if let s = model.activeSession {
        TerminalStore.shared.debugScroll(s.activeTabID, up: up, lines: lines)
      }
    ...
    case "gitCheckout":
      if let s = model.activeSession {
        _ = Git.checkout(s.url, arg)
        s.gitBranch = Git.branch(s.url)
      }
    case "gitBranchNew":
      if let s = model.activeSession {
        _ = Git.createBranch(s.url, arg)
        s.gitBranch = Git.branch(s.url)
      }
    case "gitBranchDel":
      if let s = model.activeSession { _ = Git.deleteBranch(s.url, arg, force: false) }
...
enum DebugState {
  static func capture(to path: String, _ model: AppModel) {
    ...
    if let ed = ActiveEditor.current {
      root["editorDirty"] = ed.isDirty
      root["editorTextLen"] = ed.debugText.count
      root["editorCaretLine"] = ed.debugCaretLine
      root["editorSelText"] = ed.debugSelectedText
      root["findCount"] = ed.debugFindCount
      root["findCurrent"] = ed.debugFindCurrent
    }
    ...
    if let ed = ActiveEditor.current {
      ...
      root["status"] = [
        "ln": lc.line, "col": lc.column, "eol": ed.lineEnding,
    ...
    if let s = model.activeSession {
      ...
      if let t = s.activeTab {
        var at: [String: Any] = ["kind": t.kind.rawValue, "title": t.title, "exited": t.exited]
        if t.kind == .terminal,
