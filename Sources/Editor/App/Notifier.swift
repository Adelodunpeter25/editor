...
/// notifications aren't authorized. `willPresent` lets a banner show even while Editor is frontmost
...
final class Notifier: NSObject, UNUserNotificationCenterDelegate {
    ...
    func post(sessionID: String, sessionName: String, tabID: String, tabTitle: String,
              body: String, fallback: @escaping () -> Void) {
        let center = UNUserNotificationCenter.current()
    ...
    /// Show the banner even when Editor is the active app (you may be in a different session/tab).
